...
def extract_version_from_filename(
    ...
    :param migration_script_filename_format: migration script filename
...
async def migrate_if_needed(
    ...
    migration_files = sorted(os.listdir(migration_script_dir))
    ...
    for filename in migration_files:
        ...
        if version and version > current_version:
            filepath = os.path.join(migration_script_dir, filename)
            with open(filepath, "r") as f:
                sql_content = f.read()
            await apply_migration(conn, version, sql_content)

    if version > current_version:
        # todo: also clean redis data
        pass
