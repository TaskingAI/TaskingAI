...
def _build_yi_message(message: ChatCompletionMessage):
...
def _build_yi_chat_completion_payload(
    ...
    formatted_messages = [_build_yi_message(msg) for msg in messages]
    ...
    logger.debug(f"_build_yi_chat_completion_payload: {payload}")
    return payload


class YiChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_yi_header(credentials)
        payload = _build_yi_chat_completion_payload(
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Yi content filter triggered, content was omitted")
