...
def _build_xai_message(message: ChatCompletionMessage):
...
def _build_xai_chat_completion_payload(
    ...
    formatted_messages = [_build_xai_message(msg) for msg in messages]
...
class XaiChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_xai_header(credentials)
        payload = _build_xai_chat_completion_payload(
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Xai content filter triggered, content was omitted")
