...
def _build_moonshot_message(message: ChatCompletionMessage):
...
def _build_moonshot_chat_completion_payload(
    ...
    formatted_messages = [_build_moonshot_message(msg) for msg in messages]
...
class MoonshotChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_moonshot_header(credentials)
        payload = _build_moonshot_chat_completion_payload(
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Moonshot content filter triggered, content was omitted")
