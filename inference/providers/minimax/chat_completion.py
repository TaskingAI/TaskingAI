...
def _model_dump_str(function: ChatCompletionFunction, **kwargs: Any):
    return {
        "name": function.name,
        "description": function.description,
        "parameters": str(function.parameters.model_dump(**kwargs)),
    }


def _build_minimax_message(message: ChatCompletionMessage):
...
def _build_minimax_chat_completion_payload(
    ...
    formatted_messages = [_build_minimax_message(msg) for msg in messages]
    ...
    if functions:
        payload["tools"] = [{"type": "function", "function": _model_dump_str(f)} for f in functions]
    return payload


class MinimaxChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_minimax_header(credentials)
        payload = _build_minimax_chat_completion_payload(
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Minimax content filter triggered, content was omitted")
    ...
    def stream_extract_chunk_data(self, sse_data: Dict, **kwargs) -> Optional[Dict]:
        if not sse_data.get("choices") or sse_data["choices"][0].get("delta") is None:
            return None
    ...
    def stream_extract_finish_reason(self, chunk_data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        if chunk_data and chunk_data.get("finish_reason"):
    ...
    def stream_handle_function_calls(
        ...
        delta = chunk_data.get("delta", {}) if chunk_data else None
