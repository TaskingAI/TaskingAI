...
def _build_localai_message(message: ChatCompletionMessage):
...
def _build_localai_chat_completion_payload(
    ...
    formatted_messages = [_build_localai_message(msg) for msg in messages]
...
class LocalaiChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        api_url = build_url(credentials.LOCALAI_HOST, "/v1/chat/completions")
        headers = {"Content-Type": "application/json"}
        payload = _build_localai_chat_completion_payload(
