...
def _build_sensetime_message(message: ChatCompletionMessage):
...
def _build_sensetime_chat_completion_payload(
    ...
    formatted_messages = [_build_sensetime_message(msg) for msg in messages]
...
class SensetimeChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_sensetime_header(credentials)
        payload = _build_sensetime_chat_completion_payload(
    ...
    def extract_function_calls(self, data: Dict, **kwargs) -> Optional[List[ChatCompletionFunctionCall]]:
        message_data = data if data else None
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "sensitive":
            raise_provider_api_error("Sensetime content filter triggered, content was omitted")
    ...
    def stream_extract_chunk(
        ...
        content = chunk_data.get("delta", {}) if chunk_data else None
        if content:
    ...
    def stream_handle_function_calls(
        ...
        delta = chunk_data if chunk_data else None
        if delta and delta.get("tool_calls"):
            tool_call = delta["tool_calls"][0]
            toll_call_index = delta["index"]
