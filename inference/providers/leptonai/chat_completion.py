...
def _build_leptonai_message(message: ChatCompletionMessage):
    if message.role in [
...
def _build_leptonai_chat_completion_payload(
    ...
    formatted_messages = [_build_leptonai_message(msg) for msg in messages]
...
class LeptonaiChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    def prepare_request(
        ...
        headers = build_leptonai_header(credentials)
        payload = _build_leptonai_chat_completion_payload(
