...
def _build_azure_openai_message(message: ChatCompletionMessage):
...
def _build_azure_openai_chat_completion_payload(
    ...
    formatted_messages = [_build_azure_openai_message(msg) for msg in messages]
    ...
    logger.debug(f"_build_azure_openai_chat_completion_payload: {payload}")
    return payload


class AzureOpenaiChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        api_url = build_azure_chat_url(credentials)
        headers = build_azure_openai_header(credentials)
        payload = _build_azure_openai_chat_completion_payload(
