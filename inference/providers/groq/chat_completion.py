...
def _build_groq_message(message: ChatCompletionMessage):
...
def _build_groq_chat_completion_payload(
...
class GroqChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Content filter triggered, content was omitted")
