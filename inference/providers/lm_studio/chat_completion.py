...
def _build_lm_studio_message(message: ChatCompletionMessage):
...
def _build_lm_studio_chat_completion_payload(
    ...
    formatted_messages = [_build_lm_studio_message(msg) for msg in messages]
...
class LmStudioChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        api_url = build_url(credentials.LM_STUDIO_HOST, "/v1/chat/completions")
        headers = {"Content-Type": "application/json"}
        payload = _build_lm_studio_chat_completion_payload(
