...
def _build_deepseek_message(message: ChatCompletionMessage):
...
def _build_deepseek_chat_completion_payload(
    ...
    formatted_messages = [_build_deepseek_message(msg) for msg in messages]
...
class DeepseekChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_deepseek_header(credentials)
        payload = _build_deepseek_chat_completion_payload(
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Deepseek content filter triggered, content was omitted")
