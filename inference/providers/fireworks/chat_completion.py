...
def _build_fireworks_message(message: ChatCompletionMessage):
...
def _build_fireworks_chat_completion_payload(
    ...
    formatted_messages = [_build_fireworks_message(msg) for msg in messages]
    ...
    logger.debug(f"_build_fireworks_chat_completion_payload: {payload}")
    return payload


class FireworksChatCompletionModel(BaseChatCompletionModel):
    def __init__(self):
        super().__init__()
    ...
    async def prepare_request(
        ...
        headers = build_fireworks_header(credentials)
        payload = _build_fireworks_chat_completion_payload(
    ...
    def extract_finish_reason(self, data: Dict, **kwargs) -> Optional[ChatCompletionFinishReason]:
        ...
        if finish_reason == "content_filter":
            raise_provider_api_error("Fireworks content filter triggered, content was omitted")
    ...
    def stream_handle_function_calls(
        ...
        if delta and delta.get("tool_calls"):
            ...
            if toll_call_index == function_calls_content.index:
                ...
                function_calls_content.arguments_strs.append(tool_call_function.get("arguments", ""))
                function_calls_content.names.append(tool_call_function.get("name", ""))
