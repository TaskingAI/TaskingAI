...
async def sse_stream_response(
    ...
    async def generator(first_chunk, sse_response_generator, chunk_handler):
        # yield first chunk if it is not an error chunk
        yield f"data: {json.dumps(first_chunk)}\n\n"

        async for response_dict in sse_response_generator:
            # remove empty values
    ...
    return StreamingResponse(
        generator(
            ...
            sse_response_generator=sse_generator,
