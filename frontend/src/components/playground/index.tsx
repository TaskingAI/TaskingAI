...
import { Select, Button, Checkbox, Input, Drawer, Spin, Modal, Tag, Radio, Collapse } from 'antd'
...
import CreateCollection from '../createCollection/index.js';
...
function Playground() {
    ...
    const modelsColumns = [
        {
            ...
            render: (text, record) =>
                <div>
                    <p className='table-text'>{text || 'Untitled Model'}</p>
                    <span>{record.model_id}</span><CopyOutlined className='icon-copy' onClick={() => handleCopy(record.model_id)} />
                </div>
            ,
        },
        {
    ...
    const modalTableColumn = [
        {
            ...
            render: (text, record) =>
                <div>
                    <p className='table-text'>{text || 'Untitled Collection'}</p>
                    <span>{record.collection_id}</span><CopyOutlined className='icon-copy' onClick={() => handleCopy(record.collection_id)} />
                </div>
            ,
        },
        {
    ...
    const actionColumn = [
        {
            ...
            render: (text, record) =>
                <div>
                    ...
                    <span>{record.action_id}</span><CopyOutlined className='icon-copy' onClick={() => handleCopy(record.action_id)} />
                </div>
            ,
        },
        {
    ...
    //     if (fontColor === 'orange') {
    //         const container = document.querySelectorAll('#lottie-container');
    ...
    const fetchAssistantsList = async () => {
        const res = await getAssistantsList(100, 0)
    ...
    //     if (scrollTop === 0 && contentHasMore && contentTalk.length !== 0) {
    //         const params = {
    //             limit: 10,
    //             order: 'desc',
    ...
    const initialFunction = async () => {
        ...
        if (assistantId) {
            ...
            const res = await getListChats(assistantId, params)
    ...
    const fetchHistoryMessage = async (assistantId, chatId, param) => {
        ...
        const res = await getHistoryMessage(assistantId, chatId, param)
    ...
    const handleEditAssistant = async () => {
        ...
        // const tag1 = tools.map(item => item.id)
    ...
    const handleGenerateMessage = async () => {
        ...
        // const assistantId1 = assistantId[0].split('-')[1]
        let source;
        if (stream || debug) {
    ...
    const handleListChats = async () => {
        ...
        const res = await getListChats(id)
        setListChats(res.data)
        setChatId(res.data[0]?.chat_id)
        const param = {
            order: 'asc',
    ...
    const handleRecordsAssistantSelected = (value, selectedRows) => {
    ...
    const handleLodaMore = async () => {
        ...
        const res = await getListChats(id, params)
    ...
    return (
        <Spin spinning={loading}>
            ...
            <Drawer
                ...
                onClose={handleCancel} title='Edit Assistant' placement="right" open={OpenDrawer} size='large' footer={<ModalFooterEnd handleOk={() => handleRequest(drawerName, inputValues)} onCancel={handleCancel} />}>
            ...
            <ModelModal ref={childRef} open={modelOne} handleSetModelOne={handleModalCancel} getOptionsList={fetchModelsList} modelType='chat_completion'></ModelModal>
            ...
            <Modal closeIcon={<img src={closeIcon} alt="closeIcon" className='img-icon-close' />} centered footer={[
                ...
                <ModalTable updatePrevButton={updateActionPrevButton} defaultSelectedRowKeys={selectedActionsRows} mode='multiple' handleRecordsSelected={handleActionRecordsSelected} ifSelect={true} columns={actionColumn} hasMore={hasActionMore} id='action_id' dataSource={actionList} onChildEvent={handleChildActionEvent}></ModalTable>
            </Modal>
            <Modal closeIcon={<img src={closeIcon} alt="closeIcon" className='img-icon-close' />} centered onCancel={handleModalClose} footer={[
                ...
                <ModalTable name="model" defaultSelectedRowKeys={Array.isArray(selectedRows) ? selectedRows : [selectedRows]} updatePrevButton={updateModelPrevButton} handleRecordsSelected={handleRecordsSelected} ifSelect={true} columns={modelsColumns} hasMore={hasModelMore} id='model_id' dataSource={options} onChildEvent={handleChildModelEvent}></ModalTable>
            </Modal>
            ...
            <Modal closeIcon={<img src={closeIcon} alt="closeIcon" className='img-icon-close' />} onCancel={handleAssistantModalClose} centered footer={[
                <div className='footer-group' style={{ justifyContent: 'flex-end' }} key='footer'>
                    {/* <Button key="model" icon={<PlusOutlined />} onClick={handleCreateAssistantId} className='cancel-button'>
                        New Assistant
                    </Button> */}
                    <div>
                        <span className='select-record'>
