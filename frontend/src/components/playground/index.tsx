...
import {
  ...
  getHistoryMessage,
  deleteChatItem,
} from "@/axios/playground";
import {
  getAssistantDetail,
  updateAssistant,
  getAssistantsList,
} from "@/axios/assistant";
...
function Playground() {
  ...
  const { assistantTableColumn, modelsTableColumn, collectionTableColumn } =
    CommonComponents();
  ...
  const [groupedMessages, setGroupedMessages] = useState({
    role: "Assistant",
    content: { text: [{ event_step: "", color: "", event_id: "" }] },
    useId: "user",
  });
  ...
  useEffect(() => {
    const handleClickOutside = (event: any) => {
      if (
        !(
          settingModal.current && settingModal.current.contains(event.target)
        ) &&
        !(settingIcon.current && settingIcon.current.contains(event.target))
      ) {
  ...
  const combineObjects = (item: any, arr: any) => {
    ...
    let str = "";
    const checkBoxValue1 = JSON.parse(
      localStorage.getItem("checkedValues") as string,
    ) || [1, 2];
    if (item.object === "MessageGenerationLog") {
      setLottieAnimShow(true);
      const previousItems = arr.slice(0, arr.length - 1);
      const currentIsDuplicate = previousItems.some(
        (existingItem: any) => existingItem.event_id === item.event_id,
      );
      if (!currentIsDuplicate) {
        const indexToInsert = Math.max(
          updatedGroupedMessages.content.text.length - 1,
          0,
        );
        item.color = "orange";
        updatedGroupedMessages.content.text.splice(indexToInsert, 0, item);
      } else {
        const duplicateIndex = updatedGroupedMessages.content.text.findIndex(
          (existingItem) => existingItem.event_id === item.event_id,
        );
        if (duplicateIndex !== -1) {
      ...
      item.color = "green";
      str += item.delta;
      updatedGroupedMessages.content.text[
        updatedGroupedMessages.content.text.length - 1
      ].event_step += str;
    } else if (
      item.object === "Message" &&
      checkBoxValue1.indexOf(1) === -1 &&
      checkBoxValue1.indexOf(2) !== -1
    ) {
      updatedGroupedMessages.content.text[
        updatedGroupedMessages.content.text.length - 1
      ].event_step = item.content.text;
  ...
  const fetchAssistantsList = async () => {
    ...
    setOptionList(data);
    const id = assistantId[0].split("-")[1]
      ? assistantId[0].split("-")[1]
      : assistantId[0];
  ...
  const handleEditAssistant = async () => {
    ...
    setSelectedActionsRows(
      tools.map((item: any) => {
        return { type: item.type, value: item.id };
      }),
    );
  ...
  const handleRequest = async () => {
    const inputValueMap = (
      drawerAssistantRef.current?.getRetrievalSelectedList() || []
    )
      .map((item: any) => {
        return {
          type: "collection",
    ...
    const inputPluginValues = (
      drawerAssistantRef.current?.getActionSelectedList() || []
    )
    ...
    const params = {
      model_id: Array.isArray(selectedRows)
        ? selectedRows[0].slice(-8)
        : selectedRows,
      name: drawerName || "",
  ...
  const handleNewChat = async () => {
    ...
    try {
      ...
      const { data } = res;
      setListChats((prevValues) => [
        { chat_id: data.chat_id, created_timestamp: data.created_timestamp },
        ...prevValues,
      ]);
  ...
  const handleInputPromptChange = (index: number, newValue: string) => {
    setSystemPromptTemplate((prevValues) =>
      prevValues.map((item, i) => (i === index ? newValue : item)),
    );
  };
  ...
  const handleGenerateMessage = async (contentTalk1?: any) => {
    ...
    if (
      contentTalk1 !== "flag" &&
      contentTalk[contentTalk.length - 1]?.role.toLowerCase() === "assistant"
    ) {
    ...
    const checkBoxValue1 =
      JSON.parse(localStorage.getItem("checkedValues") as string) ||
      checkBoxValue;
    ...
    const params = {
      system_prompt_variables: systemPromptVariables
        ? JSON.parse(systemPromptVariables)
        : {},
      stream: stream,
      debug: debug,
    };
    ...
    if (stream || debug) {
      source = new SSE(
        `${origin}/${project_base_url}/assistants/${id}/chats/${chatId}/generate`,
        {
          headers: {
  ...
  const handleRecordsAssistantSelected = (
    _value: string[],
    selectedRows: any[],
  ) => {
  ...
  const handleCollectionSelected = (value: any, selectedRows: any[]) => {
    ...
    const tag = selectedRows.map(
      (item) => item.name + "-" + item.collection_id,
    );
    setSelectedRetrievalRows(tag);
  };
  ...
  const handleClickDebug = (item: any) => {
    if (item.event_step === "Error Occurred") {
      ...
      delete item.color;
      const event1 = item.event
        .split("_")
        .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
        .join(" ");
      const event2 =
        item.event_step.charAt(0).toUpperCase() + item.event_step.slice(1);
      setCollapseLabel2(`${event1}: ${event2}`);
      if (!item.event_id) return;
      const data1 = debugArray1.find(
        (item1) => item1.event_id === item.event_id,
      );
      const data2: any = debugArray2.find(
        (item1) => item1.event_id === item.event_id,
      );
      if (data1) {
        delete data1.color;
        ...
        const label =
          data1.event_step.charAt(0).toUpperCase() + data1.event_step.slice(1);
        const lable1 = data1.event
          .split("_")
          .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
          .join(" ");
        ...
        const label =
          data2.event_step.charAt(0).toUpperCase() + data2.event_step.slice(1);
        const lable1 = data2.event
          .split("_")
          .map((word: string) => word.charAt(0).toUpperCase() + word.slice(1))
          .join(" ");
        setCollapseLabel1(`${lable1}: ${label}`);
  ...
  return (
    <Spin spinning={loading}>
      {!assistantId ? (
        <div className={styles["selectAssistant"]}>
          {<PlayGroundImg className={styles.svg} />}
          <div className={styles["select-assistant"]}>
            {t("projectPlaygroundSelectAssistantDesc")}
          </div>
          <div className={styles["header-news"]}>
            <div className={styles["plusParent"]}>
              <Button
                ...
                onClick={handleSelectAssistantID}
              >
                {t("projectPlaygroundSelectAssistant")}
              </Button>
            </div>
          </div>
        </div>
            ...
            <div className={styles["top"]}>
              ...
              {!assistantId && (
                <div className={styles["select-desc"]}>
                  {t("projectPlaygroundSelectAssistantInfo")}
                </div>
              )}
              ...
              <div className={styles["chats"]}>
                <div className={styles["chat-message"]}>
                  {listChats?.map((item, index) => (
                    <div
                      key={index}
                      className={`${styles.functionaliconsParent} ${
                        chatId === item.chat_id && styles.chatId
                      }`}
                      onClick={() => handleOpenChat(item.chat_id)}
                    >
                      <ChatIcon
                        className={styles["functionalicons"]}
                      ></ChatIcon>
                  ...
                  {!loadMoreHasMore && noPreviousChat && (
                    <div className={styles["lineParent"]}>
                      <div className={styles["frameChild"]} />
                      <div className={styles["noPreviousChat1"]}>
                        {t("projectPlaygroundNoPreviousChat")}
                      </div>
                      <div className={styles["frameChild"]} />
                    </div>
                  )}
            ...
            <div className={styles["header-top"]}>
              <div className={styles["header-left"]}>
                <span className={styles["chat"]}>
                  {t("projectPlaygroundChat")}
                </span>
                <span className={styles["desc"]}>{chatId}</span>
                <CopyOutlined
                  className="icon-copy"
                  onClick={() => handleCopy(chatId)}
                />
              </div>
              ...
              >
                <Button icon={<DeleteIcon />} className="cancel-button">
                  {t("projectPlaygroundDeleteChat")}
                </Button>
              </div>
            </div>
            {!chatId && (
              <div className={styles["content-center"]}>
                <div className={styles["content-img"]}>
                  <PlaygroundImg />
                  <div className={styles["waiting-for-configuration"]}>
                    {t("projectPlaygroundWaitingForConfiguration")}
                  </div>
                </div>
              </div>
            )}
            {chatId && (
              <Spin spinning={contentTalkLoading}>
                <div className={styles["content-talk"]} ref={contentRef}>
                  ...
                  {!contentHasMore && noPreviousMessage && (
                    <div className={styles["lineParent"]}>
                      <div className={styles["frameChild"]} />
                      <div className={styles["noPreviousChat1"]}>
                        {t("projectPlaygroundNoPreviousMessage")}{" "}
                      </div>
                      <div className={styles["frameChild"]} />
                    </div>
                  )}
                  ...
                  {contentTalk.map((item, index) => (
                    <div className={styles["message"]} key={index} ref={divRef}>
                      ...
                      {typeof item.content.text === "string" && (
                        <div
                          className={`${styles.text1} ${
                            item.role === "user" ? styles.userInfo : ""
                          }`}
                      ...
                      {typeof item.content.text === "object" && (
                        <div
                          className={`text1 ${
                            item.role === "user" ? styles.userInfo : ""
                          }`}
                        >
                          {item.content.text.map(
                            (item1: any, index1: number) => (
                              <div
                                key={index1}
                                className={`${
                                  item1.color === "orange" &&
                                  index === contentTalk.length - 1
                                    ? "orange"
                                    : "green"
                                } ${
                                  index1 === item.content.text.length - 1 &&
                                  styles.lastItem
                                }`}
                              >
                                {item1.color === "orange" &&
                                ...
                                item1.event_step !== "" ? (
                                  ...
                                  item1.color !== "orange" && (
                                    <div
                                      onClick={() => handleClickDebug(item1)}
                                      style={{
                                        display: "flex",
                                        alignItems: "center",
                                      }}
            ...
            >
              ...
              <div className={styles["button-group"]}>
                <div style={{ display: "flex" }}>
                  <Button
                    ...
                    onClick={handleSendAndGenerateMessage}
                    loading={sendGenerateLoading}
                  >
                    Send and Generate
                  </Button>
                  <div
                    className={`${styles.formbuttoncancel} ${
                      sendButtonLoading ? styles.loading : ""
                    }`}
                  ...
                  >
                    ...
                    <div className={styles["text1"]}>
                      {t("projectPlaygroundSend")}
                    </div>
                  </div>
                  <div
                    className={`${styles.formbuttoncancel} ${styles.button1} ${
                      generateButtonLoading ? styles.loading : ""
                    }`}
                  ...
                  >
                    ...
                    <div className={styles["text1"]}>
                      {t("projectPlaygroundChatGenerate")}
                    </div>
                  </div>
                </div>
                <div
              ...
              >
                ...
                <div className={styles["desc"]}>
                  {t("projectPlaygroundGenerationOptionsDesc")}
                </div>
                <Checkbox.Group
                ...
                <TextArea
                  style={{ height: "300px" }}
                  placeholder={`{\n   key: value\n}`}
                  value={systemPromptVariables}
                  onChange={(e) => setSystemPromptVariable(e.target.value)}
                ></TextArea>
              </div>
            </div>
          </div>
      ...
      <Drawer
        closeIcon={
          <img src={closeIcon} alt="closeIcon" className="img-icon-close" />
        }
        className={styles["assistant-drawer"]}
        width={1280}
        onClose={handleCancel}
        title={t("projectEditAssistant")}
        placement="right"
        open={OpenDrawer}
        size="large"
        footer={
      ...
      >
        <DrawerAssistant
          handleNewBundle={handleNewBundle}
          retrievalConfig={retrievalConfig}
          topk={topk}
          maxTokens={maxTokens}
          handleMaxToken={handleMaxToken}
          handleToks={handleToks}
          bundilesList={bundilesList}
          handleNewActionModal={handleNewActionModal}
          handleNewCollection={handleNewCollection}
          selectedCollectionList={selectedRetrievalRows}
          actionHasMore={hasActionMore}
          actionList={actionList}
          collectionHasMore={hasMore}
          ref={drawerAssistantRef}
          handleRetrievalConfigChange1={handleRetrievalConfigChange1}
          retrievalList={retrievalList}
          selectedActionsRows={selectedActionsRows}
          inputValue1={inputValueOne}
          inputValue2={inputValueTwo}
          handleMemoryChange1={handleMemoryChange1}
          memoryValue={memoryValue}
          handleAddPromptInput={handleAddPrompt}
          drawerName={drawerName}
          systemPromptTemplate={systemPromptTemplate}
          handleDeletePromptInput={handleDeletePromptInput}
          handleInputPromptChange={handleInputPromptChange}
          handleInputValueOne={handleInputValueOne}
          handleInputValueTwo={handleInputValueTwo}
          selectedRows={selectedRows}
          handleSelectModelId={handleSelectModelId}
          handleChangeName={handleChangeName}
          drawerDesc={drawerDesc}
          handleDescriptionChange={handleDescriptionChange}
          selectedRetrievalRows={selectedRetrievalRows}
        ></DrawerAssistant>
      </Drawer>
      <ModelModal
      ...
      <Modal
          ...
          <div className="footer-group" key="footer-group">
            ...
            <div>
              <span className="select-record">
                {recordsSelected1.length}{" "}
                {recordsSelected1.length > 1
        ...
        title={t("projectAssistantRetrievalPlaceHolder")}
        open={openModalTable}
        width={1000}
      ...
      >
        <ModalTable
          ...
          defaultSelectedRowKeys={selectedRetrievalRows}
          hangleFilterData={hangleFilterData}
          mode="multiple"
          handleRecordsSelected={handleCollectionSelected}
          ifSelect={true}
          columns={collectionTableColumn}
          dataSource={retrievalList}
          hasMore={hasMore}
          id="collection_id"
      ...
      {/* <Modal closeIcon={<img src={closeIcon} alt="closeIcon" className={styles['img-icon-close']} />} centered footer={[
                <div className='footer-group' key='footer'>
                    ...
                    <div>
                        <span className='select-record'>
                            {recordsActionSelected.length}  {recordsActionSelected.length > 1 ? `${t('projectItemsSelected')}` : `${t('projectItemSelected')}`} selected
                        </span>
            ...
            ]} title={t('projectSelectAction')} open={modalActionTableOpen} width={1000} className={`modal-inner-table ${styles['retrieval-model']}`} onCancel={handleActionModalClose}>
      ...
      >
        <ModalTable
          ...
          defaultSelectedRowKeys={
            Array.isArray(selectedRows) ? selectedRows : [selectedRows]
          }
          updatePrevButton={updateModelPrevButton}
          handleRecordsSelected={handleRecordsSelected}
          ifSelect={true}
          columns={modelsTableColumn}
          hasMore={hasModelMore}
          id="model_id"
          dataSource={options}
      ...
      <Drawer
        ...
        title="Bulk Create Action"
        placement="right"
        open={OpenActionDrawer}
        size="large"
        footer={
          <ModalFooterEnd
            handleOk={() => handleActionRequest()}
            onCancel={handleActionCancel}
          />
        }
      >
        <ActionDrawer
          showTipError={tipSchema}
          onhandleTipError={onhandleTipError}
          schema={schema}
          onSchemaChange={handleSchemaChange}
          onRadioChange={onRadioChange}
          onChangeCustom={handleCustom}
          onChangeAuthentication={hangleChangeAuthorization}
      ...
      <Modal
        ...
        onCancel={handleAssistantModalClose}
        centered
        footer={[
          <div
          ...
          >
            <div>
              ...
              <Button
                key="cancel"
                onClick={handleAssistantModalClose}
                className={`cancel-button ${styles.cancelButton}`}
              >
                {t("cancel")}
              </Button>
              <Button
                key="submit"
                onClick={handleAssistantModalClose1}
                className="next-button"
        ...
        title={t("projectPlaygroundSelectAssistant")}
        open={openAssistantModalTable}
        width={1000}
        className={`modal-inner-table ${styles.model1}`}
      >
        <ModalTable
          name="Assistant"
          ifAllowNew={true}
          updatePrevButton={updatePrevButton}
          defaultSelectedRowKeys={defaultSelectedAssistant}
          handleRecordsSelected={handleRecordsAssistantSelected}
          ifSelect={true}
          columns={assistantTableColumn}
          hasMore={modelHasMore}
          id="assistant_id"
          dataSource={optionList}
          onChildEvent={handleChildAssistantEvent}
        ></ModalTable>
      </Modal>
      <CreateCollection
        handleFetchData={() =>
          fetchDataRetrievalData({ limit: retrievalLimit || 20 })
        }
        handleModalCloseOrOpen={() => setOpenCollectionDrawer(false)}
        OpenDrawer={openCollectionDrawer}
      ></CreateCollection>
      <Drawer
        width={700}
        open={contentDrawer}
        ...
        title={t("projectPlaygroundChatCompletion")}
      >
        <Collapse
          ...
          items={[
            {
              ...
              children: (
                <div className={styles["content-drawer"]}>
                  <div className={styles["content"]}>
                    <CopyOutlined
                      className={styles["icon-copy"]}
                      onClick={() => handleCopy(chatCompletionCall)}
                    />
                  </div>
                  <TextArea
                    autoSize={true}
                    value={chatCompletionCall}
                    disabled
                  />
                </div>
              ),
        ...
        ></Collapse>
        <Collapse
          className={styles["collapse-drawer"]}
          expandIconPosition="end"
          items={[
            {
              key: "2",
              ...
              children: (
                <div className={styles["content-drawer"]}>
                  <div className={styles["content"]}>
                    <CopyOutlined
                      className={styles["icon-copy"]}
                      onClick={() => handleCopy(chatCompletionResult)}
                    />
                  </div>
                  <TextArea
                    autoSize={true}
      ...
      <Drawer
        ...
        onClose={handleCloseContentErrorDrawer}
              ...
              children: (
                <div className={styles["content-drawer"]}>
                  <div className={styles["content"]}>
                    <CopyOutlined
                      className="icon-copy"
                      onClick={() => handleCopy(errorContent)}
                    />
                  </div>
      ...
      <CreatePlugin
        ...
        handleCloseModal={handleClosePluginModal}
      ></CreatePlugin>
      <DeleteModal
        title={t("projectPlaygroundDeleteChatUpper")}
        projectName={chatId}
        open={OpenDeleteModal}
        describe={`${"deleteItem"} ${t("projectPlaygroundChatLow")} ${chatId}`}
